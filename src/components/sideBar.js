...
import { sideMenu } from '../constants/sideBar';
...
const SideBar = () => {
  ...
  return (
    <div className="absolute md:relative lg:relative w-full md:w-80 z-10">
      ...
      >
        <div className="hidden xl:flex justify-start p-6 items-center space-x-3">
          ...
          <p className="text-2xl leading-6 text-white">Resto</p>
        </div>
        {sideMenu.map((menuItem, index) => (
          <div
            className="flex flex-col justify-start items-center   px-6 border-b border-gray-600 w-full  "
            key={menuItem.label}
          >
            <button
            ...
            >
              <Link
                to={!menuItem.children && menuItem.to}
              ...
              {menuItem.children && (
                <svg
                  id="icon1"
                  className={`${
                    menu[index] ? '' : 'rotate-180'
                  } transform duration-100 
                ${menuItem.children ? '' : 'hidden'}`}
                  width={24}
                  height={24}
                  viewBox="0 0 24 24"
                  fill="none"
              ...
              >
                {menuItem.children.map((child) => (
                  <Link
                    ...
                    onClick={() => setShow(false)}
                  >
                    <p className="text-base leading-4  ">{child.label}</p>
                  </Link>
                ))}
              </div>
            )}
        ...
        <div className="flex flex-col justify-between items-center h-full pb-6   px-6  w-full  space-y-32 ">
          <div className=" flex justify-between items-center w-full absolute px-2 pb-5 bottom-0">
